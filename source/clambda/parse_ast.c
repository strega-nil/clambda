...
void
parse_ast_match (struct parse_ast const *ast, void const *data,
                 parse_ast_var_match_t *var, parse_ast_abs_match_t *abs,
                 parse_ast_app_match_t *app)
{
  switch (ast->tag)
    {
    case PARSE_AST_VAR:
...
static void
format_var (struct parse_ast_var const *ast, void *file)
{
  fputs (ast->name, file);
}
static void
format_abs (struct parse_ast_abs const *ast, void *file)
{
  /*
    note, because it isn't obvious: ast->expression is a pointer to heap
    that's why we don't do &ast->expression
  */
  format (file, "(/%.%)", string_format, &ast->variable,
          parse_ast_format, ast->expression);
}
static void
format_app (struct parse_ast_app const *ast, void *file)
{
  format (file, "% %", parse_ast_format, ast->callee,
          parse_ast_format, ast->argument);
}

void
parse_ast_format (FILE *f, struct parse_ast const *ast)
{
  parse_ast_match (ast, f, format_var, format_abs, format_app);
}

void
parse_ast_print (struct parse_ast const *ast) {
  parse_ast_format (stdout, ast);
}

static struct parse_ast *
allocate (struct parse_ast ast)
{
  struct parse_ast *ret = calloc (1, sizeof (*ret));
  *ret = ast;
  return ret;
}

static char *
allocate_str (char const *old)
{
...
struct parse_ast
parse_ast_make_var (char const *name)
{
...
struct parse_ast
parse_ast_make_abs (char const *var, struct parse_ast expr)
{
  struct parse_ast ret;
...
static void
delete_var (struct parse_ast_var *ast, void *ignore)
{
  (void) ignore;
  free (ast->name);
}
static void
delete_abs (struct parse_ast_abs *ast, void *ignore)
{
  (void) ignore;
  free (ast->variable);
...
static void
delete_app (struct parse_ast_app *ast, void *ignore)
{
  (void) ignore;
...
void
parse_ast_delete (struct parse_ast ast)
{
